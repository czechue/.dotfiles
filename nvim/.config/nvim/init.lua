...
require('lazy').setup({
  ...
  { -- Highlight, edit, and navigate code
    ...
    -- The old 'master' branch is archived and breaks on Neovim 0.12+; 'main' is the rewrite.
    branch = 'main',
    build = ':TSUpdate',
      ...
      vim.api.nvim_create_autocmd('FileType', {
        ...
        callback = function(args)
          ...
          local function start()
            ...
            -- Ruby indents better with vim's regex-based indentexpr.
            if lang ~= 'ruby' then
              vim.bo[args.buf].indentexpr = "v:lua.require'nvim-treesitter'.indentexpr()"
            end
          end
          -- language.add returns nil (rather than erroring) when no parser is installed
          local ok, added = pcall(vim.treesitter.language.add, lang)
          if ok and added then
            start()
          elseif vim.tbl_contains(require('nvim-treesitter').get_available(), lang) then
            -- Auto-install missing parsers, then enable for the waiting buffer
